 ...
 * Copyright: 2022 Naotoshi Fujita
 */
(function(factory){typeof define==='function'&&define.amd?define(factory):factory()})(function(){'use strict';function empty$1(array){array.length=0}
...
function typeOf(type,subject){return typeof subject===type}
function isObject(subject){return!isNull(subject)&&typeOf("object",subject)}
var isArray=Array.isArray;apply(typeOf,"function");var isString=apply(typeOf,"string");var isUndefined=apply(typeOf,"undefined");function isNull(subject){return subject===null}
...
function removeClass(elm,classes){toggleClass(elm,classes,!1)}
function unit(value){return isString(value)?value:value?value+"px":""}
var PROJECT_CODE="splide";function assert(condition,message){if(!condition){throw new Error("["+PROJECT_CODE+"] "+(message||""))}}
var min=Math.min,max=Math.max,floor=Math.floor,ceil=Math.ceil,abs=Math.abs;function pad(number){return number<10?"0"+number:""+number}
var CLASS_SLIDE_ROW=CLASS_SLIDE+"__row";var CLASS_SLIDE_COL=CLASS_SLIDE+"--col";var DEFAULTS={rows:1,cols:1,dimensions:[],gap:{}};function Dimension(options){function normalize(){var rows=options.rows,cols=options.cols,dimensions=options.dimensions;return isArray(dimensions)&&dimensions.length?dimensions:[[rows,cols]]}
function get(index){var dimensions=normalize();return dimensions[min(index,dimensions.length-1)]}
function getAt(index){var dimensions=normalize();var rows,cols,aggregator=0;for(var i=0;i<dimensions.length;i++){var dimension=dimensions[i];rows=dimension[0]||1;cols=dimension[1]||1;aggregator+=rows*cols;if(index<aggregator){break}}
assert(rows&&cols,"Invalid dimension");return[rows,cols]}
return{get:get,getAt:getAt}}
function Layout(Splide2,gridOptions,Dimension){var _EventInterface=EventInterface(Splide2),on=_EventInterface.on,destroyEvent=_EventInterface.destroy;var Components=Splide2.Components,options=Splide2.options;var resolve=Components.Direction.resolve;var forEach=Components.Slides.forEach;function mount(){layout();if(options.slideFocus){on(EVENT_VISIBLE,onVisible);on(EVENT_HIDDEN,onHidden)}}
function destroy(){forEach(function(Slide){var slide=Slide.slide;toggleTabIndex(slide,!1);getRowsIn(slide).forEach(function(cell){removeAttribute(cell,"style")});getColsIn(slide).forEach(function(colSlide){cover(colSlide,!0);removeAttribute(colSlide,"style")})});destroyEvent()}
function layout(){forEach(function(Slide){var slide=Slide.slide;var _Dimension$get=Dimension.get(Slide.isClone?Slide.slideIndex:Slide.index),rows=_Dimension$get[0],cols=_Dimension$get[1];layoutRow(rows,slide);layoutCol(cols,slide);getColsIn(Slide.slide).forEach(function(colSlide,index){colSlide.id=Slide.slide.id+"-col"+pad(index+1);if(Splide2.options.cover){cover(colSlide)}})})}
function layoutRow(rows,slide){var rowGap=gridOptions.gap.row;var height="calc("+100/rows+"%"+(rowGap?" - "+unit(rowGap)+" * "+(rows-1)/rows:"")+")";getRowsIn(slide).forEach(function(rowElm,index,rowElms){style(rowElm,"height",height);style(rowElm,"display","flex");style(rowElm,"margin","0 0 "+unit(rowGap)+" 0");style(rowElm,"padding",0);if(index===rowElms.length-1){style(rowElm,"marginBottom",0)}})}
function layoutCol(cols,slide){var colGap=gridOptions.gap.col;var width="calc("+100/cols+"%"+(colGap?" - "+unit(colGap)+" * "+(cols-1)/cols:"")+")";getColsIn(slide).forEach(function(colElm,index,colElms){style(colElm,"width",width);if(index!==colElms.length-1){style(colElm,resolve("marginRight"),unit(colGap))}})}
function cover(colSlide,uncover){var container=child(colSlide,"."+CLASS_CONTAINER);var img=child(container||colSlide,"img");if(img&&img.src){style(container||colSlide,"background",uncover?"":"center/cover no-repeat url(\""+img.src+"\")");style(img,"display",uncover?"":"none")}}
...
function toggleTabIndex(slide,add){getColsIn(slide).forEach(function(colSlide){setAttribute(colSlide,"tabindex",add?0:null)})}
function onVisible(Slide){toggleTabIndex(Slide.slide,!0)}
function onHidden(Slide){toggleTabIndex(Slide.slide,!1)}
return{mount:mount,destroy:destroy}}
function Grid(Splide2,Components2,options){var _EventInterface2=EventInterface(Splide2),on=_EventInterface2.on,off=_EventInterface2.off;var Elements=Components2.Elements;var gridOptions={};var Dimension$1=Dimension(gridOptions);var Layout$1=Layout(Splide2,gridOptions,Dimension$1);var modifier=CLASS_ROOT+"--grid";var originalSlides=[];function mount(){init();on(EVENT_UPDATED,init)}
...
function destroy(){if(isActive()){var slides=Elements.slides;Layout$1.destroy();originalSlides.forEach(function(slide){removeClass(slide,CLASS_SLIDE_COL);append(Elements.list,slide)});remove(slides);removeClass(Splide2.root,modifier);empty(slides);push(slides,originalSlides);empty(originalSlides);off(EVENT_REFRESH)}}
function refresh(){Splide2.refresh()}
function layout(){if(isActive()){Layout$1.mount()}}
function build(){var outerSlides=[];var row=0,col=0;var outerSlide,rowSlide;originalSlides.forEach(function(slide,index){var _Dimension$1$getAt=Dimension$1.getAt(index),rows=_Dimension$1$getAt[0],cols=_Dimension$1$getAt[1];if(!col){if(!row){outerSlide=create(slide.tagName,CLASS_SLIDE);outerSlides.push(outerSlide)}
rowSlide=buildRow(rows,slide,outerSlide)}
buildCol(cols,slide,rowSlide);if(++col>=cols){col=0;row=++row>=rows?0:row}});return outerSlides}
function buildRow(rows,slide,outerSlide){var tag=slide.tagName.toLowerCase()==="li"?"ul":"div";return create(tag,CLASS_SLIDE_ROW,outerSlide)}
function buildCol(cols,slide,rowSlide){addClass(slide,CLASS_SLIDE_COL);append(rowSlide,slide);return slide}
function shouldBuild(){if(options.grid){var rows=gridOptions.rows,cols=gridOptions.cols,dimensions=gridOptions.dimensions;return rows>1||cols>1||isArray(dimensions)&&dimensions.length>0}
return!1}
function isActive(){return hasClass(Splide2.root,modifier)}
return{mount:mount,destroy:destroy}}
if(typeof window!=="undefined"){window.splide=window.splide||{};window.splide.Extensions=window.splide.Extensions||{};window.splide.Extensions.Grid=Grid}})