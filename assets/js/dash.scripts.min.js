...
navLinksAnimation(){let navLinks=this.nav.querySelectorAll(".c-header-nav .c-header-nav-link");if(!navLinks)return;navLinks.forEach((item,key)=>{item.style.transitionDelay=(key*.125)+"s"})}};class Schedule{constructor(form){this.scheduleForm=document.querySelector(form)}
init(){if(!this.scheduleForm)return;console.log(this.scheduleForm)}};(new CHeader(".sidebar-toggler",".sidebar")).init();(new Schedule("#jsNewSchedule")).init();/*!
